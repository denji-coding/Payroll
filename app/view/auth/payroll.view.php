...
.filter-tag-green:hover {
    background: #f0fdf4;
    color: #166534;
}
.filter-label .filter-checkbox:checked + .filter-tag-green {
    background: #dcfce7;
    color: #166534;
    border-color: #86efac;
}
.filter-label .filter-checkbox:checked + .filter-tag-green .filter-check {
    visibility: visible;
    color: #22c55e;
}
...
.filter-tag-blue:hover {
    background: #eff6ff;
    color: #1e40af;
}
.filter-label .filter-checkbox:checked + .filter-tag-blue {
    background: #dbeafe;
    color: #1e40af;
    border-color: #93c5fd;
}
.filter-label .filter-checkbox:checked + .filter-tag-blue .filter-check {
    visibility: visible;
    color: #3b82f6;
}
...
.filter-tag-purple:hover {
    background: #faf5ff;
    color: #7e22ce;
}
.filter-label .filter-checkbox:checked + .filter-tag-purple {
    background: #f3e8ff;
    color: #7e22ce;
    border-color: #d8b4fe;
}
.filter-label .filter-checkbox:checked + .filter-tag-purple .filter-check {
    visibility: visible;
    color: #a855f7;
}
</style>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateDuration() {
    ...
    if (startDate.value && endDate.value) {
      ...
      durationDisplay.textContent = workingDays > 0 ? `${workingDays} working day(s)` : 'No working days in range';
    } else {
      ...
      .then(data => {
        ...
        if (data.status === 'success') {
          ...
          data.data.forEach((emp, index) => {
            ...
            const totalDeductions = totalBenefits + leaveDeduction;
            const showValue = isProcessed ? (v) => v.toFixed(2) : () => '--';

            html += `
              <tr class="border-b hover:bg-[#f2f8f2] even:bg-[#cde4cd] fade-in-slide" 
                  data-employee="${emp.employee_no}" 
                  data-manager-id="${emp.manager_id || ''}"
                  data-employee-id="${emp.employee_id || ''}"
                  data-hr-id="${emp.hr_id || ''}"
                  data-user-type="${emp.user_type || 'employee'}">
                ...
                <td class="leaveDeduction p-3 align-middle text-left font-medium">${showValue(leaveDeduction)}</td>
    ...
    document.querySelectorAll('.autoCalcBtn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // Update row UI
            ...
            row.querySelector('.leaveDeduction').textContent = leaveDeduction.toFixed(2);
  ...
  // CANCEL CALCULATION
    document.querySelectorAll('.cancelCalcBtn').forEach((btn) => {
      ...
      // Reset all calculated fields
        ...
        row.querySelector('.leaveDeduction').textContent = '--';
  ...
  // MANUAL CALCULATE → SAVE button
    document.getElementById('savePayBtn').addEventListener('click', (e) => {
      ...
      const { gross, net, sss, pagibig, philhealth, leaveDeduct, totalDeductions } = calculatedManual;
      ...
      row.querySelector('.benefit').dataset.pagibig = pagibig.toFixed(2);
      row.querySelector('.benefit').dataset.philhealth = philhealth.toFixed(2);
      row.querySelector('.leaveDeduction').textContent = leaveDeduct.toFixed(2);
...
function processPayroll() {
  ...
  rows.forEach(row => {
    ...
    if (checkbox && checkbox.checked && checkbox.getAttribute('data-processed') !== '1') {
      ...
      const employeeId = row.dataset.employeeId || null;
      const managerId = row.dataset.managerId || null;
      const hrId = row.dataset.hrId || null;
      const userType = row.dataset.userType || 'employee';
      ...
      payload.push({
        ...
        leave_deduction: parseFloat(row.querySelector('.leaveDeduction')?.textContent.trim()) || 0,
        total_deductions: (parseFloat(benefitCell?.textContent.trim()) || 0) + (parseFloat(row.querySelector('.leaveDeduction')?.textContent.trim()) || 0),
...
<section class="bg-white shadow-sm rounded-lg border-2 border-green-200 p-6 space-y-6"
  ...
  <div class="flex flex-col gap-4 mb-4">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <span class="text-lg font-semibold text-[#478547]">Select Employees, Managers & HR for Payroll</span>
  ...
  <div class="relative w-64">
    ...
    <input
      ...
      class="flex h-10 w-full placeholder:ml-[10px] rounded-md border border-input bg-background px-[50px] py-2 pl-8 text-base placeholder:text-[#478547] ring-offset-[#f8fbf8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16a249] focus-visible:ring-offset-2 disabled:opacity-50 md:text-sm"
          placeholder="Search employee, manager, or HR..."
    ...
    <!-- Filter Checkboxes (Right aligned, under search) -->
    <div class="flex justify-end">
      <div class="flex items-center gap-1 bg-white border border-gray-200 rounded-lg p-1 shadow-sm">
        <span class="text-xs text-gray-500 font-medium px-2 border-r border-gray-200 mr-1">
          <i class="bi bi-funnel text-gray-400"></i> Filter by Role
        </span>
        <label class="filter-label">
          <input type="checkbox" id="filterEmployees" class="filter-checkbox" onchange="filterByUserType()">
          <span class="filter-tag filter-tag-green">
            <i class="bi bi-check2 filter-check"></i>
            Employees
          </span>
        </label>
        <label class="filter-label">
          <input type="checkbox" id="filterManagers" class="filter-checkbox" onchange="filterByUserType()">
          <span class="filter-tag filter-tag-blue">
            <i class="bi bi-check2 filter-check"></i>
            Managers
          </span>
        </label>
        <label class="filter-label">
          <input type="checkbox" id="filterHR" class="filter-checkbox" onchange="filterByUserType()">
          <span class="filter-tag filter-tag-purple">
            <i class="bi bi-check2 filter-check"></i>
            HR
          </span>
        </label>
...
<script>
  ...
  rows.forEach(row => {
    ...
    const userType = row.dataset.userType || 'employee';
    const showEmployees = document.getElementById('filterEmployees') ? document.getElementById('filterEmployees').checked : false;
    const showManagers = document.getElementById('filterManagers') ? document.getElementById('filterManagers').checked : false;
    const showHR = document.getElementById('filterHR') ? document.getElementById('filterHR').checked : false;
    
    let typeMatches = false;
    
    // If no checkboxes are checked, show all types
    if (!showEmployees && !showManagers && !showHR) {
      typeMatches = true;
    } 
    // If only employees checkbox is checked, show only employees
    else if (showEmployees && !showManagers && !showHR) {
      typeMatches = (userType === 'employee');
    }
    // If only managers checkbox is checked, show only managers
    else if (!showEmployees && showManagers && !showHR) {
      typeMatches = (userType === 'manager');
    }
    // If only HR checkbox is checked, show only HR
    else if (!showEmployees && !showManagers && showHR) {
      typeMatches = (userType === 'hr');
    }
    // If multiple checkboxes are checked, show matching types
    else {
      if (showEmployees && userType === 'employee') typeMatches = true;
      if (showManagers && userType === 'manager') typeMatches = true;
      if (showHR && userType === 'hr') typeMatches = true;
    }
  ...
  if (searchTerm && visibleCount === 0) {
    ...
    const filterManagers = document.getElementById('filterManagers');
    const filterHR = document.getElementById('filterHR');
    
    if (filterEmployees) {
      filterEmployees.addEventListener('change', filterTable);
    }
    if (filterManagers) {
      filterManagers.addEventListener('change', filterTable);
    }
    if (filterHR) {
      filterHR.addEventListener('change', filterTable);
    }
  });

// Filter by user type (Manager/HR checkboxes) - calls filterTable which includes checkbox logic
