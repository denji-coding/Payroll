...
<div class="flex min-h-screen overflow-hidden">
    ...
    <main id="mainContent" class="flex-1 p-6 bg-gray-100 transition-margin duration-300 ease-in-out" style="margin-left: 256px;">
        ...
        <!-- Existing Payroll Header -->
<div>
    <span class="text-2xl font-bold tracking-tight">Payroll</span>
    <p class="text-gray-600 mb-2">Select employees and calculate net pay</p>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function fetchPayrollData() {
    ...
    fetch(`../app/api/branch_payroll-api.php?start_date=${start}&end_date=${end}`)
      ...
      .then(data => {
        ...
        if (data.status === 'success') {
          ...
          data.data.forEach((emp, index) => {
            ...
            html += `
              <tr class="border-b hover:bg-[#f2f8f2] even:bg-[#cde4cd] fade-in-slide" data-employee="${emp.employee_no}">
...
function processPayroll() {
  ...
  rows.forEach(row => {
    ...
    if (checkbox && checkbox.checked && checkbox.getAttribute('data-processed') !== '1') {
      ...
      payload.push({
        employee_no: row.children[1].textContent.trim(),
